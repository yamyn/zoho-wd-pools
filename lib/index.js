...
class WorkdriveController {
    ...
    async [getAccessToken](pool, creds) {
        try {
            ...
            if (error.message === `getaddrinfo ENOTFOUND accounts.zoho.${creds.domain}`) {
                throw new Error(`Invalid domain - ${creds.domain}`)
            }
            throw error;
        }
    }
    ...
    addConection = async (pool, creds) => {
        ...
        const ttl = this.isTestCreds(pool) ? this[config].stdTTLTestCreds : this[config].stdTTLCreds;
        this[credsCache].set(this[parseKey](pool).creds(), creds, ttl);
    ...
    files = {
        ...
        toTrash: async (pool, params) => {
            return this[withToken](WorkdriveService.filesService.toTrashFiles, pool, params);
        },
