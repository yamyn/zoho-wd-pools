...
class WorkdriveController {
    ...
    parseMetaStr = (fileId, shareId) => `${fileId}${this[config].metaMark.bwn}${shareId}${this[config].metaMark.end}`;
    parseDownloadUri = str => {
        const urls = {};
        const arr = str.split(this[config].metaMark.end);
        arr.forEach(str => {
            const [id, linkId] = str.split(this[config].metaMark.bwn);

            urls[id] = this.parseDownloadUrlById(id, linkId);
        });

        return urls;
    };

    parseDownloadUrlById = (fileId, downLinkId) =>
        `https://files.zohoexternal.com/public/workdrive-external/download/${fileId}?x-cli-msg=%7B%22linkId%22%3A%22${downLinkId}%22%7D`;
